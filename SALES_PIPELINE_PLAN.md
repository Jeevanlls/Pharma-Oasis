...
- ✅ **B3 — Printable / PDF quotation** (DONE 2026-06-25). A clean, print-optimised quotation document at **`/quotes/:id/print`** (Pharma Oasis header, quote # + date + expiry, "prepared for" customer block, line-item table, total, notes, validity footer) with a **Print / Save as PDF** button (uses the browser print dialog — no server-side PDF library). Role-aware: customers load it via `/api/quotes/:id`, admins via the new **`GET /api/admin/quotes/:id`** (returns items + customer, secrets stripped). Linked from the customer quote-detail page ("Print / Download quotation"), the admin Quotes page ("Quotation" button), and the **quote response email** now includes a "View / print your quotation" button (`SITE_URL/quotes/:id/print`). _Future: attach a real generated PDF to the email rather than a link (needs a server-side PDF lib)._

**Verified:** B1+B2 — 13-assertion end-to-end test (accept → one linked confirmed order with copied line snapshots + carried total; duplicate-accept returns the same order; decline creates no order). B3 — 8-assertion test (admin single-quote endpoint returns items + customer with secrets stripped; no route clash with `/:id/versions`; customer endpoint feeds the doc; cross-customer access blocked; print route serves). Type-checks clean; builds succeed.
